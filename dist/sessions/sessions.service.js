...
let SessionsService = class SessionsService {
    constructor(sessionRepo, messageRepo, fileRepo, cabinetRepo) {
        ...
        for (const session of sessions) {
            const unreadCount = await this.messageRepo.count({
                where: {
                    ...
                    isRead: false,
                    senderCabinetId: (0, typeorm_2.Brackets)(qb => qb.where('senderCabinetId != :cabinetId', { cabinetId })),
                },
            });
    ...
    async getOrCreateSession(cabinetA_id, cabinetB_id) {
    ...
    async getMessages(sessionId, cabinetId) {
        ...
        if (!session) {
            throw new common_1.ForbiddenException('无权访问该会话');
        }
    ...
    async sendMessage(sessionId, file, senderCabinetId, uploaderId) {
        const session = await this.sessionRepo.findOne({
            where: [
        ...
        if (!session) {
            throw new common_1.ForbiddenException('无权向该会话发送消息');
        }
    ...
    async downloadFile(messageId, cabinetId) {
...
exports.SessionsService = SessionsService = __decorate([
    ...
    __param(3, (0, typeorm_1.InjectRepository)(cabinet_entity_1.Cabinet)),
